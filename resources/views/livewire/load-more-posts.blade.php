<div>
    <div class="posts my-6 sm:grid md:grid-cols-2 lg:grid-cols-3 gap-6 space-y-6 sm:space-y-0">
        @forelse ($posts as $post)
            <div class="w-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                ...
                <div class="p-5">
                    <a wire:click="getSpecificPosts({{ $post->category_id }})" href="{{ route('frontend.blog-category-post', $post->category_id) }}">
    ...
    @if ($rest_posts > 0)
        <div class="text-center py-6">
            <button type="button"
                    wire:click="loadMore"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2">Load More</button>
        </div>
        @else
    @endif
</div>
