<x-guest-layout>
    <div class="content max-w-screen-xl mx-auto px-4">
        ...
        <div class="category-title">
            <h2 class="text-center text-4xl font-extrabold capitalize py-6">{{ $tag_name }}</h2>
        </div>

        {{-- blog category specific posts cards --}}
        <div class="posts my-6 sm:grid md:grid-cols-2 lg:grid-cols-3 gap-6 space-y-6 sm:space-y-0">
            @forelse ($posts as $data)
                <div class="w-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <a href="{{ route('frontend.blog-post', [$data->id, $data->slug]) }}">
                        <img class="rounded-t-lg"
                            src="{{ asset('storage/thumbnails/'.$data->thumbnail) }}"
                            alt="thumbnail" />
                    </a>
                    <div class="p-5">
                        <a href="{{ route('frontend.blog-category-post', $data->category_id) }}">
                        ...
                        <a href="{{ route('frontend.blog-post', [$data->id, $data->slug]) }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            Read more
